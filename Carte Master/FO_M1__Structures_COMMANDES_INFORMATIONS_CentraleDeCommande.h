...
enum DCT_Obst_Type {DCT_unique, DCT_balayage, DCT_balayage_plus_proche}; // Choisir le type de detection, unique, balayage, plus proche par balayage
...
typedef struct COMMANDES COMMANDES ;      // Cette structure contient toutes les commandes envoyées par la centrale de commande
...
{           
   ...
   BYTE  DCT_Obst_Resolution;      // Paramètre résolution angulaire de la détection d'obstacle
	 enum  DCT_Obst_Type Type_DCT_Obst; // Paramètre type de détection d'obstacle
...
typedef struct INFORMATIONS INFORMATIONS ;       // Cette structure contient toutes les informations susceptibles d'être transmises à la centrale de commande
